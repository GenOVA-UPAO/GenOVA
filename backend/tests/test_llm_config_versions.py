...
GROQ_8B = {"provider": "groq", "model_id": "llama-3.1-8b-instant"}
GROQ_70B = {"provider": "groq", "model_id": "llama-3.3-70b-versatile"}
LABELS = {
...
def test_describe_changes_primary_fallbacks_and_flags():
    before = _cfg(texto=FLASH, fallbacks=[GROQ_8B])
    after = _cfg(texto=GROQ_70B, fallbacks=[], video=True)
    texts = [c["text"] for c in versions.describe_changes(before, after, LABELS)]
    assert texts == [
        "Texto: DeepSeek V4 Flash → Llama 3.3 70B",
        "Texto, respaldos: Llama 3.1 8B → ninguno",
...
def test_record_change_skips_noop_and_caps_entries(kv, monkeypatch):
    ...
    for i in range(5):
        a = _cfg(texto=FLASH if i % 2 else GROQ_8B)
        b = _cfg(texto=GROQ_8B if i % 2 else FLASH)
...
def test_record_change_never_raises(monkeypatch):
    ...
    assert versions.record_change(_cfg(texto=FLASH), _cfg(texto=GROQ_8B), _User()) is None
...
def test_profiles_crud_and_validation(kv, monkeypatch):
    user = _User()
    p = versions.create_profile("  Económico  ", _cfg(texto=GROQ_8B), user)
...
def test_put_records_history_and_undo_restores(api):
    ...
    assert any(ch["text"] == "Texto: DeepSeek V4 Flash → Llama 3.3 70B" for ch in entry["changes"])
    assert entry["source"] == "manual"

    same = _put(c, GROQ_70B)
    ...
    assert redo.json()["config"]["defaults"]["texto"]["model_id"] == GROQ_70B["model_id"]
...
def test_restore_unknown_entry_and_bad_target(api):
    ...
    _put(c, FLASH)
    entry = _put(c, GROQ_70B).json()["history_entry"]
...
def test_profiles_api_flow(api):
    ...
    assert "Texto: Llama 3.3 70B → DeepSeek V4 Flash" in texts
...
def test_admin_endpoints_forbidden_for_users(api):
    ...
    body = {"provider": "groq", "model_id": "llama-3.1-8b-instant"}
...
def test_admin_test_model_fake_and_validation(api, monkeypatch):
    ...
    no_key = c.post(
        "/api/admin/llm-config/test-model",
        json={"provider": "groq", "model_id": "llama-3.1-8b-instant"},
    ).json()
