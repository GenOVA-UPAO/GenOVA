...
from prometheus.prompts.elaborate_prompts import (
    CODE_ONLY,
    RECURSOS_META,
    prompt_codigo,
    prompt_html,
...
class GenerateElaborateRequest(BaseModel):
    resource_type: int
    concept: str
...
def _retrieve_contexto(db: Session, query: str, upload_ids: list[str]) -> str:
    ...
    if contexto:
        logger.info("RAG retrieved %d chunks for ELABORATE concept=%r", len(chunks), query[:60])
    return contexto
...
def _generate_two_step(n: int, concept: str, contexto: str) -> tuple[dict | list, str]:
    ...
    try:
        ...
        logger.warning("JSON parse failed for ELABORATE %d, retrying with strict prompt", n)
        ...
        retry = generar_texto(
            ...
            "texto", max_tokens=3000,
        )
        try:
            json_data = parse_json(retry)
        except Exception:
            logger.warning("JSON retry also failed for ELABORATE %d, using raw text", n)
...
def generate_elaborate_resource(
    ...
    try:
        if n in CODE_ONLY:
            ...
            html, _ = validate_and_repair(html, "elaborate", n)
        ...
        logger.exception("Error generating ELABORATE resource %d", n)
        raise HTTPException(
