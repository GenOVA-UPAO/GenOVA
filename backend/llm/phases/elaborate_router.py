...
def _retrieve_contexto(db: Session, query: str, upload_ids: list[str]) -> str:
    ...
    if contexto:
        logger.info(
            "RAG retrieved chunks", fase="ELABORATE", chunk_count=len(chunks), concept=query[:60]
        )
    return contexto
...
def _generate_two_step(n: int, concept: str, contexto: str) -> tuple[dict | list, str]:
    ...
    try:
        ...
        logger.warning(
            "JSON parse failed, retrying with strict prompt", fase="ELABORATE", resource_type=n
        )
        ...
        try:
            ...
            logger.warning(
                "JSON retry also failed, using raw text", fase="ELABORATE", resource_type=n
            )
...
def generate_elaborate_resource(
    ...
    try:
        ...
        logger.exception("error generating resource", fase="ELABORATE", resource_type=n)
        raise HTTPException(
