...
class OvaJob(Base):
    ...
    ova_id = Column(UUID(as_uuid=True), ForeignKey("ovas.id", ondelete="SET NULL"),
...
class OvaJobResource(Base):
    ...
    id = _job_pk()
    job_id = Column(UUID(as_uuid=True), ForeignKey("ova_jobs.id", ondelete="CASCADE"),
                    nullable=False, index=True)
    ...
    ova_phase_id = Column(UUID(as_uuid=True),
                          ForeignKey("ova_phases.id", ondelete="SET NULL"), nullable=True)
