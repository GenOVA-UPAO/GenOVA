...
try:
    ...
    SMTP_PORT = 465
SMTP_USER = os.getenv("SMTP_USER", "soporte.genova.upao@gmail.com")
# Google App password: spaces are stripped
SMTP_PASSWORD = os.getenv("SMTP_PASSWORD", "ccjmnnfdelwcyemf").replace(" ", "")

def send_reset_email(to_email: str, reset_link: str, full_name: str | None = None):
    try:
        ...
        logger.error(f"Failed to send reset password email to {to_email}: {str(e)}")
        raise e
