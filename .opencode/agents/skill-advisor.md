...
2. Si está → invoca el contenido de `.agents/skills/find-skills/SKILL.md` como guía para ejecutar: `npx skills search "<descripción de tarea>"`.
...
- ❌ Ejecutar `npx skills install` — eso lo hace el humano o el leader con confirmación.
- ❌ Modificar `skills-lock.json` — solo lo toca el CLI `npx skills`.
- ❌ Recomendar skills de sources no confiables sin marcarlas `pending_review`.
