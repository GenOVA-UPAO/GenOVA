...
function NodeBadge({
  ...
  const color = videoWarning
    ? 'bg-amber-500'
    : colors[node.name.length % colors.length]
  ...
  return (
    <div className="flex items-center gap-4 px-6 py-4 border-b border-border/50 last:border-0 hover:bg-accent/30 transition-colors">
      <div
        className={`h-10 w-10 rounded-2xl shrink-0 ${color} shadow-sm border border-black/10 flex items-center justify-center`}
      >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-foreground flex items-center gap-2">
          {node.name}
          {videoWarning && (
            <span className="bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-widest border border-amber-200">
              ⚠ API Key faltante
            </span>
          )}
        </p>
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 border-b border-border/50 last:border-0 hover:bg-accent/30 transition-colors">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-bold text-muted-foreground bg-muted/50 rounded-md px-1.5 py-0.5 border border-border/50 tracking-widest uppercase">
              {node.role}
            </span>
          </div>
      ...
      <div className="flex items-center gap-4 shrink-0 mt-3 sm:mt-0">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`text-[10px] font-bold tracking-widest uppercase ${isOn ? 'text-emerald-600' : 'text-muted-foreground'}`}
          >
            {isOn ? 'Activo' : 'Pausado'}
          </span>
          <Toggle
...
export function PlatformNodesCard() {
  ...
  useEffect(() => {
    if (data) {
      ...
      setRounds(Number(data.config.ova_reflection_rounds ?? 1))
    }
  }, [data])
  ...
  const handleSave = () => {
    save.mutate(
      { ...draft, ova_reflection_rounds: String(rounds) } as Record<
        string,
        string
      >,
      {
  ...
  const alwaysOnNodes = (data?.nodes ?? []).filter(
    (n) => n.always_on && n.id !== 'video',
  )
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-muted-foreground mt-1">
            Activa o desactiva agentes del grafo de generación. Los cambios
            aplican en ~30s.
          </p>
        </div>
        <Button
