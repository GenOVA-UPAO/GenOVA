    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {provider.badge && (
            <span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wide ${provider.badgeColor}`}>
              {provider.badge}
            </span>
          )}
        </div>
      ...
      <ArrowRight size={14} className="shrink-0 mt-1 text-muted-foreground/30 group-hover:text-primary transition-colors" />
    </button>
  )
}

export function ConnectProviderModal({ open, onClose, onSelectProvider }) {
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md gap-0 p-0 overflow-hidden">
        <div className="px-5 pt-5 pb-4 border-b border-border/60 bg-muted/20">
          <DialogHeader>
            <DialogTitle className="text-base font-bold">Conectar proveedor</DialogTitle>
        ...
        <div className="mx-4 mb-4 flex items-start gap-2 rounded-lg border border-border/40 bg-muted/30 px-3 py-2.5">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Para acceder a modelos de <span className="font-semibold text-foreground">Anthropic, OpenAI, Google, Mistral</span> y más —
            conecta <span className="font-semibold text-foreground">OpenRouter</span> con tu cuenta.
          </p>
        </div>
