import { Article, Brain, Code, type Icon, Lock, Robot } from '@phosphor-icons/react'
...
> = {
  ...
  razonamiento: {
    Icon: Brain,
    bar: 'bg-accent-brand/60',
    tint: 'bg-accent-brand/[.04]',
  },
}
  ...
  return (
    <div className="space-y-2">
      ...
      {Object.keys(taskLabels).map((tipo) => {
        ...
        const value =
          cur.provider && cur.model_id ? `${cur.provider}::${cur.model_id}` : ''
        ...
        return (
          ...
          >
            <div
              className={`absolute inset-y-0 left-0 w-[3px] rounded-l-xl ${bar}`}
            />
              ...
              <div className="flex items-center gap-2">
                    ...
                    {Object.entries(catalog).map(([provider, models]) => {
                      ...
                      return (
                        <SelectGroup key={provider}>
                          <SelectLabel
                            className={`text-[10px] ${down ? 'opacity-40' : ''}`}
                          >
                          ...
                          {(Array.isArray(models) ? models : []).map(
                            (m: unknown) => {
                              const mid =
                                typeof m === 'string'
                                  ? m
                                  : (m as Record<string, string>).model_id
                              const mlabel =
                                typeof m === 'string'
                                  ? m
                                  : (m as Record<string, string>).label || mid
                              const pricing =
                                typeof m === 'string'
                                  ? null
                                  : (m as Record<string, unknown>).pricing
                              const ctx =
                                typeof m === 'string'
                                  ? null
                                  : formatContextLength(
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <Input
                    ...
                    disabled={locked}
                    onChange={(e) =>
                      setTipoTimeout(tipo, Number(e.target.value))
                    }
