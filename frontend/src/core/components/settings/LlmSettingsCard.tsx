  ...
  return (
    <section className="glass-card rounded-3xl p-6 sm:p-8 space-y-6">
      {error && !loading ? (
        <Alert
          variant="destructive"
          className="bg-destructive/5 border-destructive/20 rounded-2xl"
        >
        ...
        <>
          ...
          <Tabs defaultValue="assign" className="space-y-6">
            <TabsList className="bg-muted/30 p-1 rounded-xl glass-card border border-border/50">
              ...
              >
                <SlidersHorizontal size={16} weight="bold" className="mr-2" />{' '}
              ...
              <TabsTrigger
                ...
                className="rounded-lg font-bold text-xs"
              >
                <GridFour size={16} weight="bold" className="mr-2" /> Catálogo
                de modelos
                {catalogFull.length > 0 && (
                  <span className="ml-2 rounded-full bg-primary/20 text-primary px-1.5 py-0.5 text-[9px]">
                    {catalogFull.length}
                  </span>
                )}
              </TabsTrigger>
                ...
                <div className="rounded-2xl border border-blue-200/50 bg-blue-50/50 p-4 flex items-center gap-3 glass-card shadow-sm">
                  ...
                  <div>
                    <p className="text-sm font-bold text-blue-900">
                      Usando configuración de Plataforma UPAO
                    </p>
                    <p className="text-xs font-medium text-blue-800/80 mt-0.5">
                      Agrega tu API Key para personalizar los modelos de cada
                      tarea.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center gap-4 rounded-3xl border-2 border-dashed border-border/60 bg-muted/20 px-6 py-12 text-center glass-card">
                  ...
                  <div className="space-y-1.5 max-w-sm">
                    <p className="text-base font-bold text-foreground font-display">
                      Catálogo de modelos bloqueado
                    </p>
                    <p className="text-sm font-medium text-muted-foreground">
                      Añade una API key en la pestaña{' '}
