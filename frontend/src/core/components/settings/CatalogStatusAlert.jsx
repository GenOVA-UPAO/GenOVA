...
import { PROVIDER_LABELS } from '@/core/lib/llmCatalogUtils.js'
...
/** Warning banner shown when a provider's model list couldn't be refreshed. */
export function CatalogStatusAlert({ catalogStatus, refreshing, onRetry }) {
  const down = Object.entries(catalogStatus || {}).filter(([, st]) => st && st.ok === false)
  if (down.length === 0) return null

  const names = down.map(([p]) => PROVIDER_LABELS[p] || p).join(' y ')
  const lastOk = down.map(([, st]) => _formatTimestamp(st.last_success_at)).find(Boolean)
