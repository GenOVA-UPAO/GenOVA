...
export function LlmEnginesPanel() {
  ...
  if (loading) {
    return <p className="text-xs text-muted-foreground">Cargando motores de IA…</p>
  }
  if (error) {
        ...
        {active.map((m) => (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <span className="font-semibold text-foreground truncate">{m.label || m.model_id}</span>
              <span
                className={`shrink-0 rounded-full border px-1.5 py-0.5 text-[10px] font-semibold ${
                  PROVIDER_BADGE[m.provider] || 'bg-muted text-muted-foreground border-border'
                }`}
              >
