...
function Chips({ fallbacks, models, chip, num }) {
  if (!fallbacks?.length) return <p className="text-[10px] italic text-muted-foreground/40">Sin cadena de respaldo</p>
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {fallbacks.slice(0, 3).map((f, i) => (
        <span key={i} className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-semibold border ${chip}`}>
          <span className={`text-[9px] ${num}`}>#{i + 1}</span>
          <span className="truncate max-w-[90px]">{label(f)}</span>
        </span>
      ))}
      {fallbacks.length > 3 && (
