...
import { TotpSetupCard } from '@/features/profile/components/TotpSetupCard'
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-card px-6 py-5 shadow-sm">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center">
          <div className="relative shrink-0">
            ...
            <span
              className={`absolute -bottom-1 -right-1 h-4 w-4 rounded-full border-2 border-card ${isAdmin ? 'bg-primary' : 'bg-accent-brand'}`}
            />
          </div>
          <div className="flex-1 min-w-0 space-y-0.5">
            {loading ? (
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span
                  className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${isAdmin ? 'bg-primary/10 text-primary border border-primary/25' : 'bg-accent-brand/10 text-accent-brand border border-accent-brand/25'}`}
                >
                  {role}
                </span>
              </div>
