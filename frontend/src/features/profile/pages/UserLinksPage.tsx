...
import { getCurrentUser, type MeUser } from '@/core/lib/auth/me'
import LinkRow from '@/features/ova_library/components/cards/LinkRow'
import {
  acceptLink,
  createLinkCode,
  deleteAnyLink,
...
export function UserLinksPage() {
  ...
  const load = async (current: MeUser | null = user) => {
    if (!current) return
    setLoading(true)
    try {
      ...
      toast.error(error.message || 'No se pudieron cargar los vinculos.')
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    getCurrentUser().then((current) => {
      ...
      if (can(current, 'users:link') || can(current, 'users:link:admin')) {
        const loader = can(current, 'users:link:admin')
          ? fetchAllLinks
          : fetchMyLinks
        loader()
  ...
  const handleResend = async (linkId: string) => {
    try {
      ...
      const error = err as { message?: string }
      toast.error(error.message || 'No se pudo reenviar la invitacion.')
    }
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
            ...
            {hasUserLinks ? (
              <div className="glass-card rounded-3xl border-border bg-card p-6 shadow-sm space-y-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    El estudiante ingresa este codigo en su perfil para
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  >
                    <LinkSimple size={16} weight="bold" className="mr-2" />{' '}
                    Generar codigo
                  </Button>
                  {generatedCode ? (
                    <code className="flex-1 text-center rounded-xl bg-primary/10 border border-primary/20 px-3 py-2 text-sm font-bold text-primary tracking-widest">
                      {generatedCode}
                    </code>
                  ) : null}
                </div>
          ...
          <div className="md:col-span-7 glass-card rounded-3xl border-border bg-card shadow-sm overflow-hidden flex flex-col h-full min-h-[400px]">
            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                <div className="p-10 text-center text-sm font-medium text-muted-foreground animate-pulse">
                  Cargando...
                </div>
              ) : links.length > 0 ? (
                links.map((link) => (
