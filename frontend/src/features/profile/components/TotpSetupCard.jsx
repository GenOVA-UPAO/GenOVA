...
import { z } from 'zod'
import { ShieldCheck, ShieldSlash, Copy, Check } from '@phosphor-icons/react'
...
export function TotpSetupCard({ totpEnabled: initialEnabled }) {
  const [phase, setPhase] = useState(initialEnabled ? 'enabled' : 'idle')
  ...
  const {
    ...
    reset,
  } = useForm({ resolver: zodResolver(codeSchema), defaultValues: { code: '' } })
  ...
  const startSetup = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setServerError(data?.message || 'Error al iniciar la configuración.')
        return
      }
      setSetupData(data)
    ...
    return (
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm">Autenticación en 2 pasos (2FA)</h3>
        </div>
        ...
        <Button size="sm" onClick={startSetup}>Activar 2FA</Button>
      </div>
    )
  }
    ...
    return (
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="rounded-lg bg-muted/40 p-3 space-y-2">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">URI de aprovisionamiento</p>
          <div className="flex items-center gap-1 overflow-hidden">
            <code className="text-[10px] break-all text-foreground flex-1">{setupData.provisioning_uri}</code>
            <CopyButton text={setupData.provisioning_uri} />
          </div>
          <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground mt-2">Clave secreta</p>
          <div className="flex items-center gap-1">
            <code className="text-xs font-mono text-foreground">{setupData.secret}</code>
        ...
        {backupCodes ? (
          <div className="rounded-lg border border-amber-500/30 bg-amber-50/10 p-3 space-y-1.5">
            ...
            <div className="grid grid-cols-4 gap-1">
              {backupCodes.map((c) => (
                <code key={c} className="text-xs font-mono bg-muted/60 rounded px-1.5 py-0.5 text-center">{c}</code>
              ))}
            </div>
          </div>
        ) : null}
    ...
    return (
      <div className="rounded-xl border border-green-500/30 bg-card p-5 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm">Autenticación en 2 pasos activada</h3>
        </div>
        <p className="text-xs text-muted-foreground">Tu cuenta está protegida. Para desactivar, confirma con un código de tu autenticador.</p>

        <div className="flex items-end gap-2">
          <div className="space-y-1 flex-1 max-w-[160px]">
            <Label htmlFor="disable-code" className="text-xs">Código actual</Label>
            <Input
              id="disable-code"
              type="text"
              inputMode="numeric"
        ...
        {disableError ? (
          <Alert variant="destructive"><AlertDescription>{disableError}</AlertDescription></Alert>
        ) : null}
      </div>
    )
  }
