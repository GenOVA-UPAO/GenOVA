...
@Component({
  ...
  template: `
    <div class="rounded-xl border border-border overflow-hidden shadow-md">
      ...
      @if (isTabbed) {
        <div class="flex bg-muted/30 border-b border-border">
...
export class ThemeMiniPreviewComponent {
  readonly colorMode = input.required<string>();
  readonly designMode = input.required<string>();
  ...
  get primary(): string {
    ...
    if (colorMode === "upao") return "#0A3D91";
    if (colorMode === "custom") return this.palette()?.p ?? "#0A3D91";
    return "#6D28D9";
  }

  get accent(): string {
    const colorMode = this.colorMode();
    if (colorMode === "upao") return "#F47A20";
    if (colorMode === "custom") return this.palette()?.a ?? "#38BDF8";
    return "#A78BFA";
  }
