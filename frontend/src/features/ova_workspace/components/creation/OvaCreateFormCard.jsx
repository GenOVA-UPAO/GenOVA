...
import { SlidersHorizontal, Paperclip, Swatches, Lightning } from '@phosphor-icons/react'
import { Button } from '@/core/components/ui/button'
import { Textarea } from '@/core/components/ui/textarea'
import { FileChip } from '@/features/ova_workspace/components/shared/FileChip.jsx'
import { OvaFilesModal } from '@/features/ova_workspace/components/modals/OvaFilesModal.jsx'
import { OvaThemeModal } from '@/features/ova_workspace/components/modals/OvaThemeModal.jsx'
      ...
      >
        <div className="my-auto w-full max-w-2xl space-y-5">
          ...
          <div className="text-center space-y-1.5 px-2">
            ...
            <h1 className="font-display text-[1.75rem] sm:text-3xl font-semibold leading-tight">Crear nuevo OVA</h1>
            <p className="text-sm text-muted-foreground">Describe el tema y configura los recursos a generar con IA</p>
          </div>
          ...
          <div className="rounded-2xl border border-border/70 bg-card overflow-hidden"
            ...
            {/* Hidden file input for drag-on-card fallback */}
          ...
          <p className="text-center text-[11px] text-muted-foreground/60 tracking-wide">Ctrl+Enter para generar</p>
        </div>
      </div>

      {showFiles && (
        <OvaFilesModal
          uploads={uploads} activeUploadsCount={activeUploadsCount} maxUploadFiles={maxUploadFiles}
          onFilesSelected={onFilesSelected} onRemove={onRemove}
          onClose={() => setShowFiles(false)}
        />
      )}

      {showTheme && (
        <OvaThemeModal theme={theme} onThemeChange={setTheme} onClose={() => setShowTheme(false)} />
      )}
    </div>
  )
}
