...
export function PromptPanel({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) void uploadsProps.onFilesSelected(e.target.files)
    e.target.value = ''
  }

  const hasUploads = uploadsProps.uploads.length > 0
  const canUploadMore =
    uploadsProps.activeUploadsCount < uploadsProps.maxUploadFiles
  ...
  return (
    <div className="rounded-xl border border-border bg-background p-4 sm:p-5 shadow-sm space-y-4">
          ...
          <div className="flex items-center justify-between border-t border-border px-3 py-2 bg-muted/20 rounded-b-lg">
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                disabled={
                  isGenerating || uploadsProps.disabled || !canUploadMore
                }
              ...
              <span className="text-[10px] text-muted-foreground font-medium">
                PDF, Word, PowerPoint, Audio o Imagen (
                {uploadsProps.activeUploadsCount}/{uploadsProps.maxUploadFiles})
              </span>
            </div>
      ...
      <p className="text-xs text-muted-foreground">
        Mínimo {minChars} caracteres · elige 1-4 recursos por cada una de las 5
        fases 5E.
      </p>

      <p className="rounded bg-accent-brand/10 border border-accent-brand/30 px-2.5 py-1.5 text-xs text-accent-brand shadow-sm">
        <b>Video:</b> Sin modelo disponible. Usa HeyGen, Synthesia o Sora con el
        contenido generado por GenOVA.
      </p>
