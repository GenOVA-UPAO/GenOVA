...
export function FallbackPage(): JSX.Element {
  ...
  const tasks = (config.data as { tasks?: string[] })?.tasks ?? []
  ...
  useEffect(() => {
    if (config.data) {
      const cfg = config.data as { config?: Draft; tasks?: string[] }
      setDraft(toDraft(cfg.config ?? {}, cfg.tasks ?? []))
    }
  }, [config.data])
  ...
  const handleSave = () => {
    save.mutate(toPayload(draft, tasks), {
      onSuccess: () =>
        toast.success('Cadena de fallback guardada exitosamente.'),
      onError: (e: Error) =>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 max-w-2xl text-sm font-medium text-muted-foreground">
            Orden de prioridad de modelos de IA por cada caso de uso en la
            plataforma.
          </p>
        </div>
        {isAdmin ? (
          <Button
            ...
            className="self-start gap-2 shadow-md hover:shadow-lg transition"
          >
            <ArrowsClockwise
              size={18}
              weight="bold"
              className={save.isPending ? 'animate-spin' : ''}
            />
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-blue-200/50 bg-blue-50/50 p-5 flex items-start gap-4 glass-card shadow-sm">
            ...
            <div>
              <p className="text-sm font-bold text-blue-900">
                Visualizando cadena de Plataforma UPAO (solo lectura)
              </p>
              <p className="text-xs font-medium text-blue-800/80 mt-1">
                El orden de modelos de respaldo general es administrado por
                plataforma. Puedes agregar tu propia API Key en 'Modelos de IA'
                para priorizar tus preferencias.
              </p>
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {['Primario', 'Fallback 1', 'Fallback 2'].map((label, index) => (
              <div
                key={label}
                className="rounded-2xl border-2 border-border/40 bg-card p-5 glass-card shadow-sm hover:border-primary/20 transition-colors"
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={`h-2 w-2 rounded-full ${index === 0 ? 'bg-primary' : index === 1 ? 'bg-amber-400' : 'bg-muted-foreground'}`}
                  />
                </div>
                ...
                <p className="mt-1.5 text-xs text-muted-foreground font-medium">
                  GenOVA recorre la cadena automáticamente cuando hay errores o
                  latencia excesiva.
                </p>
              </div>
            ))}
      ...
      <AnimatePresence mode="wait">
        {isAdmin && config.isError ? (
          ...
          >
            <Alert
              variant="destructive"
              className="glass-card bg-destructive/5 border-destructive/20 rounded-2xl"
            >
              ...
              <AlertDescription className="font-medium text-destructive/80">
                {(config.error as Error)?.message ||
                  'Hubo un error de conexión. Intenta actualizar la página.'}
              </AlertDescription>
            </Alert>
          </motion.div>
        ) : null}
      ...
      {isAdmin && (config.isLoading || !draft) ? (
        <div className="grid gap-5">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="h-40 animate-pulse rounded-3xl bg-muted/30 border border-border/40"
            />
          ))}
        </div>
      ) : null}
        ...
        <div className="grid gap-6">
          {tasks.map((task: string) => (
            <LlmTaskRow
              ...
              task={task}
              value={draft[task]}
              models={(catalog.data ?? []) as unknown as AdminModel[]}
              disabled={!isAdmin || save.isPending}
              onChange={(next) =>
                setDraft(
                  (current) => ({ ...(current ?? {}), [task]: next }) as Draft,
                )
              }
            />
          ))}
