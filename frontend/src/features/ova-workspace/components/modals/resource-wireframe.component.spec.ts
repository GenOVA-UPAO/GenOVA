...
describe("ResourceWireframeComponent", () => {
  it.each(ALL_KINDS)("renders a sketch for wire kind '%s' without throwing", async (kind) => {
    ...
    expect(root?.style.color.length).toBeGreaterThan(0);
  });
  ...
  it("audio renders 8 waveform bars", async () => {
    ...
    expect(container.querySelectorAll(".bg-current\\/60").length).toBe(8);
  });

  it("quiz renders 3 radio+line rows", async () => {
    const { container } = await renderWireframe("quiz");
    expect(container.querySelectorAll(".rounded-full.border-2.border-current").length).toBe(3);
  });

  it("timeline renders 4 milestone dots", async () => {
    const { container } = await renderWireframe("timeline");
    expect(
      container.querySelectorAll(".rounded-full.border-2.border-current.bg-background").length,
    ).toBe(4);
  });

  it("game renders a 3x3 board", async () => {
    const { container } = await renderWireframe("game");
    expect(container.querySelectorAll(".grid-cols-3.grid-rows-3 > div").length).toBe(9);
  });

  it("map renders 4 pins with connecting lines", async () => {
    const { container } = await renderWireframe("map");
    expect(container.querySelectorAll(".rounded-full.bg-current").length).toBe(4);
