...
function tabClass(active: boolean): string {
  return cn(
    "inline-flex h-8 min-w-0 items-center justify-center gap-1.5 rounded-md px-3 text-sm font-medium whitespace-nowrap transition-colors duration-150",
 ...
 */
export function SegmentedTabs<T extends string>({ label, value, options, onChange, className }: Readonly<Props<T>>) {
  ...
  return (
    <div role="tablist" aria-label={label} className={cn("inline-flex rounded-lg bg-muted p-1", className)}>
      {options.map((option) => {
