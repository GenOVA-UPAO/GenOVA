      ...
      <div class="space-y-3">
        ...
        <div class="rounded-xl border border-border overflow-hidden shadow-sm">
          <gn-html-preview-frame
            ...
            [className]="
              'w-full h-[60vh] min-h-[240px] max-h-[640px] border-0 block' +
              (view === 'preview' ? '' : ' hidden')
            "
            height=""
          ></gn-html-preview-frame>
          <div [class.hidden]="view !== 'code'">
            <div class="flex items-center justify-between bg-slate-800 px-3 py-2">
              <span
                class="text-[10px] font-mono font-medium uppercase tracking-wider text-slate-400"
              >
                HTML
              </span>
            ...
            <pre
              class="max-h-[480px] overflow-auto bg-slate-900 p-4 text-[11px] leading-relaxed text-slate-200"
            ><code>{{ result.html_content }}</code></pre>
          </div>
        </div>
      </div>
    }
