...
export function useChatRegeneration(ovaId: string) {
  ...
  const request = useMutation({ mutationFn: async ({ prompt, phaseIds }: { prompt: string; phaseIds: string[] }) => {
    await chat.append.mutateAsync(userChatMessage(prompt));
    const running = assistantRunningMessage();
