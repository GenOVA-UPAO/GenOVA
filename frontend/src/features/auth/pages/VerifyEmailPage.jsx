...
export function VerifyEmailPage() {
  ...
  useEffect(() => {
    if (ran.current) return // StrictMode guard: verify once (token is single-use)
    ran.current = true
    if (!token) {
      setStatus('error')
        ...
        {status === 'success' && (
          <>
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-primary/10">
              <CheckCircle size={26} weight="duotone" className="text-primary" aria-hidden="true" />
            </div>
            <h1 className="text-2xl font-semibold tracking-tight">¡Correo verificado!</h1>
            <p className="mt-2 text-sm text-muted-foreground">Tu cuenta ya está activa.</p>
            <Button type="button" className="mt-5 w-full" onClick={() => navigate('/dashboard')}>
              Ir al dashboard
            </Button>
          </>
        )}
        ...
        {status === 'error' && (
          <>
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-destructive/10">
              <WarningCircle size={26} weight="duotone" className="text-destructive" aria-hidden="true" />
            </div>
            <h1 className="text-2xl font-semibold tracking-tight">No se pudo verificar</h1>
