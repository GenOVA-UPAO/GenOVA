...
import { type RefreshCatalogResponse,refreshLlmCatalog } from "../api/llm-settings.api";
...
import { failingOwnProviders, platformStatusForUser } from "../lib/own-catalog-status";
...
/** Proveedores que siguen fallando: los de plataforma y los de las claves propias. */
export function stillFailing(result: RefreshCatalogResponse): string[] {
  const platform = failedProviders(
    platformStatusForUser(result.catalog_status, result.own_catalog_status),
  );
  return [...new Set([...platform, ...failingOwnProviders(result.own_catalog_status)])];
}
