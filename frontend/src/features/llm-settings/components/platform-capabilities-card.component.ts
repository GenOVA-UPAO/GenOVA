      ...
      @if (!loading && !error && draft) {
        <div class="rounded-3xl border border-border bg-card shadow-sm overflow-hidden glass-card">
          @for (cap of capabilities; track cap) {
            <div
              class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 border-b border-border/50 last:border-0 hover:bg-accent/30 transition-colors"
            >
              <div class="flex items-center gap-4 flex-1 min-w-0">
                <div
                  class="h-10 w-10 rounded-2xl shrink-0 bg-primary/80 shadow-sm border border-primary/20 flex items-center justify-center"
                >
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if (cap.role) {
                      <span
                        class="text-[9px] font-bold text-muted-foreground bg-muted/50 rounded-md px-1.5 py-0.5 border border-border/50 tracking-widest uppercase"
                        >{{ cap.role }}</span
                      >
                    }
                  ...
                  <p class="text-[11px] font-medium text-muted-foreground mt-0.5 leading-snug">
                    {{ cap.description }}
                  </p>
                </div>
              </div>
              <div class="flex items-center gap-3 shrink-0">
                @if (cap.always_on) {
                  <span
                    class="text-[10px] font-bold text-emerald-600 tracking-widest uppercase bg-emerald-500/10 px-2 py-1 rounded-md border border-emerald-500/20 shadow-sm"
                  >
                    Siempre activo
                  </span>
                }
                @if (!cap.always_on) {
                  ...
                  >
                    <span
                      ...
                      [ngClass]="isCapActive(cap.flag) ? 'translate-x-5' : 'translate-x-0'"
                    ></span>
                  </button>
                }
              </div>
