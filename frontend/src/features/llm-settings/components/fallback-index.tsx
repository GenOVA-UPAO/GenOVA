...
export function FallbackIndex({ index, modality }: Readonly<{ index: number; modality: string }>) {
  return (
    <div className="flex shrink-0 items-center gap-2">
      <span className="rounded-md border border-border bg-muted px-2 py-1 text-[10px] font-bold text-muted-foreground/60">
        #{index + 1}
      </span>
      {modality !== "text" ? (
      ...
      {index > 0 ? (
        <span className="hidden text-[10px] font-black text-muted-foreground/30 sm:inline">←</span>
      ) : null}
    </div>
  );
}
