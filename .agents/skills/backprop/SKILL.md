...
Still append §B entry — record that this failure mode was considered.

## OUTPUT SHAPE

Every backprop run produces:
1. §B entry (always) — appended to relevant spec in `sdd/specs/`.
2. §V entry (usually) — added to `CHECKPOINTS.md`.
3. Test file (when §V added).
4. Code fix.
5. One commit: `backprop §B.<n> + §V.<N>: <one-line cause>`.

No dashboards. No log files. Spec + CHECKPOINTS.md + git is the full history.
