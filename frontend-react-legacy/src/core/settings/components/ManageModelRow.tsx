...
export const ManageModelRow = memo(function ManageModelRow({
  model,
  locked,
  enabled,
  onToggle,
  ...
  async function handleClick() {
    ...
    setSaving(true)
    try {
      await onToggle(model.provider, model.model_id)
    } finally {
      setSaving(false)
    }
  }
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {locked && (
            ...
            >
              <title>Modelo base del sistema</title>
            </Lock>
          )}
        </div>
        {model.description && (
      ...
      {isFree ? (
        ...
        <div className="relative group/price shrink-0 flex items-center gap-1">
          ...
          <div
            className="invisible group-hover/price:visible absolute right-0 bottom-6 z-50
          ...
          >
            ...
            {model.pricing_detail.input != null && (
              <div className="flex justify-between gap-2 text-muted-foreground">
                ...
                <span className="font-mono text-foreground">
                  ${model.pricing_detail.input.toFixed(4)}
                </span>
              </div>
            )}
            {model.pricing_detail.output != null && (
              <div className="flex justify-between gap-2 text-muted-foreground">
                <span>Salida</span>
                <span className="font-mono text-foreground">
                  ${model.pricing_detail.output.toFixed(4)}
                </span>
              </div>
            )}
            {model.pricing_detail.cache_read != null && (
              <div className="flex justify-between gap-2 text-muted-foreground">
                ...
                <span className="font-mono text-foreground">
                  ${model.pricing_detail.cache_read.toFixed(4)}
                </span>
              </div>
            )}
            {model.pricing_detail.cache_write != null && (
              <div className="flex justify-between gap-2 text-muted-foreground">
                ...
                <span className="font-mono text-foreground">
                  ${model.pricing_detail.cache_write.toFixed(4)}
                </span>
              </div>
            )}
          </div>
