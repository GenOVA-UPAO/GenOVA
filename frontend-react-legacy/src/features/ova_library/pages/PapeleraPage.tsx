...
import { TrashedOvaCard } from '@/features/ova_library/components/cards/TrashedOvaCard'
import { useTrashList } from '@/features/ova_library/hooks/useTrashList'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1.5 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            OVAs movidos a la papelera. Restáuralos o elimínalos
            definitivamente.
          </p>
        </div>
      ...
      {!loading && !error && ovas.length > 0 ? (
        <label
          ...
          className="flex items-center gap-2 cursor-pointer select-none text-xs text-muted-foreground font-medium"
        >
          <Checkbox
      ...
      {!loading && !error && totalPages > 1 ? (
        <div className="flex items-center justify-between border-t border-border pt-4 px-1">
          <p className="text-xs text-muted-foreground font-medium">
            Página{' '}
            <span className="text-foreground font-bold">{currentPage}</span> de{' '}
