...
export interface DiffData {
  v1?: {
    ...
    phases?: Array<{ id: string; phase_type: string; content?: string }>
  }
  v2?: {
    version?: { version_number?: number }
    phases?: Array<{ id: string; phase_type: string; content?: string }>
...
interface VersionHistoryPanelProps {
  ...
  onDiff: (v1: string | number, v2: string | number) => Promise<DiffData>
}
...
export function VersionHistoryPanel({
  ...
  async function handleRevert(version: OvaVersion) {
    if (
      !window.confirm(
        `¿Revertir al OVA v${version.version_number}? Se perderán los cambios no guardados.`,
      )
    )
      return
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      ...
                      else if (!diffRight && v.id !== diffLeft)
                        setDiffRight(v.id)
                    } else {
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-[10px] font-mono">
            <div>
              <p className="font-semibold mb-1">
                v{diffData.v1?.version?.version_number}
              </p>
              {(diffData.v1?.phases ?? []).map((p) => (
                <div key={p.id} className="mb-2 rounded bg-muted/40 p-1.5">
                  ...
                  <pre className="mt-0.5 whitespace-pre-wrap max-h-24 overflow-auto">
                    {p.content || '—'}
                  </pre>
                </div>
              ))}
            ...
            <div>
              <p className="font-semibold mb-1">
                v{diffData.v2?.version?.version_number}
              </p>
