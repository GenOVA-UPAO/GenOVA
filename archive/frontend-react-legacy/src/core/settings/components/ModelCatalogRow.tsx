...
export function ModelCatalogRow({
  ...
  const ctx = formatContextLength(model.context_length ?? 0)
  ...
  return (
    ...
    >
      <button
        ...
        onClick={() =>
          !locked && !saving && onToggle(model.provider, model.model_id)
        }
      ...
      <span className="flex-1 truncate text-foreground inline-flex items-center gap-1 min-w-0">
        ...
        {!model.curated && (
          ...
          >
            <title>No optimizado para OVAs</title>
          </Warning>
        )}
        {locked && (
      ...
      <span
        className={`inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[9px] font-bold shrink-0 ${meta.bg} ${meta.color}`}
      >
