...
import { saveApiKey } from '@/features/ova_workspace/services/userLlmSettingsService'

interface KeyRowProps {
  provider: string
  maskedValue?: string | null
  onSaved: (keys: Record<string, string>) => void
}

const PROVIDER_META: Record<
  string,
  { label: string; placeholder: string; href: string }
> = {
  groq: {
    label: 'Groq',
    placeholder: 'gsk_...',
  ...
  runware: {
    label: 'Runware',
    placeholder: '...',
    href: 'https://runware.ai/account/api-keys',
  },
  falai: { label: 'fal.ai', placeholder: '...', href: 'https://fal.ai/keys' },
}

export function KeyRow({ provider, maskedValue, onSaved }: KeyRowProps) {
  ...
  async function handleSave() {
    ...
    setRowError(null)
    try {
      const result = (await saveApiKey(provider, value.trim())) as {
  ...
  async function handleDelete() {
    ...
    setRowError(null)
    try {
      const result = (await saveApiKey(provider, '')) as {
        api_keys: Record<string, string>
      }
      onSaved(result.api_keys)
    } catch (e) {
