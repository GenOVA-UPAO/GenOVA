...
function MiniPreview({ color }: MiniPreviewProps) {
  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden shadow-md text-left">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${primary}1A 0%, ${accent}26 100%)`,
        }}
      >
      ...
      <div className="p-2.5 bg-background space-y-1.5">
        ...
        <div
          className="rounded-md py-1 mt-1 text-center"
...
interface OvaThemeModalProps {
  theme: OvaTheme
  onThemeChange: (theme: OvaTheme) => void
  onClose: () => void
}

export function OvaThemeModal({
  theme,
  onThemeChange,
  onClose,
}: OvaThemeModalProps) {
  ...
  const themeTitle =
    draft.color === 'upao' && draft.design === 'upao'
