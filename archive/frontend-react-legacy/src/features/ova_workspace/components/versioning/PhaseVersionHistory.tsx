...
import {
  fetchPhaseVersions,
  revertPhaseVersion,
} from '@/features/ova_workspace/services/ovaEditService'
...
interface PhaseVersionHistoryProps {
  open: boolean
...
export function PhaseVersionHistory({
  open,
  onOpenChange,
  ovaId,
  phaseId,
  onReverted,
}: PhaseVersionHistoryProps) {
  const [mvs, setMvs] = useState<MicroVersion[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setLoading(true)
      try {
        const data = (await fetchPhaseVersions(ovaId, phaseId)) as {
          micro_versions?: MicroVersion[]
        }
  ...
  async function handleRevert(mv: MicroVersion) {
    if (
      !window.confirm(
        `¿Revertir el recurso a la micro-versión ${mv.minor_number}?`,
      )
    )
      return
    try {
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    {mv.created_at
                      ? new Date(mv.created_at).toLocaleString('es-PE')
                      : '—'}
                  </span>
                  <Button
                    type="button"
