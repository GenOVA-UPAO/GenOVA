...
export function ModelsPage(): JSX.Element {
  const [user, setUser] = useState<ModelsPageUser | null>(null)
  ...
  const canModels =
    !user ||
    isAdmin ||
    (user?.permissions || []).includes('ai:models:self') ||
    (user?.permissions || []).includes('ai:models:platform')
  const baseTasks = (adminHook.config.data as { tasks?: string[] })?.tasks ?? [
    'texto',
    'codigo',
    'orquestador',
    'razonamiento',
  ...
  const adminModels = (adminHook.catalog.data ?? []) as unknown as AdminModel[]
  ...
  useEffect(() => {
    if (adminHook.config.data) {
      const cfg = adminHook.config.data as {
        config?: Draft
        tasks?: string[]
      }
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {isAdmin && (
            <Button
              ...
              disabled={!draft || adminHook.save.isPending}
              className="gap-1.5 shadow-md text-xs font-bold shrink-0"
            >
              <ArrowsClockwise
                size={13}
                weight="bold"
                className={adminHook.save.isPending ? 'animate-spin' : ''}
              />
