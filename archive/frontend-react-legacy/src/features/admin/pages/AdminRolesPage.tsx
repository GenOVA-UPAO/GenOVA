...
import { RoleFormModal } from '@/features/admin/components/RoleFormModal'
...
function RegistrationModeCard() {
  ...
  useEffect(() => {
    apiFetch('/api/admin/registration-mode')
      ...
      .then((d) =>
        setTesis(
          (d.default_registration_role ?? 'usuarios_prueba') ===
            'usuarios_prueba',
        ),
      )
      .catch(() => {})
  ...
  async function toggle() {
    ...
    try {
      await apiFetch('/api/admin/registration-mode', {
        method: 'PUT',
        body: JSON.stringify({
          default_registration_role: next ? 'usuarios_prueba' : 'usuario',
        }),
      })
      setTesis(next)
    } finally {
  ...
  return (
    <div className="rounded-xl border border-border bg-background p-4 flex items-center justify-between gap-4">
      ...
      >
        <span
          className={`block h-5 w-5 rounded-full bg-white shadow-lg transition-transform duration-200
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 text-sm font-medium text-muted-foreground">
            Define los conjuntos de permisos y configuraciones de acceso para
            los diferentes perfiles del sistema.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-5">
            {roles.map((role) => (
              <div
                key={role.id}
                className="rounded-3xl border-2 border-border/40 bg-card p-6 glass-card shadow-sm hover:border-primary/20 transition"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-4">
                  <div className="flex items-center gap-4">
                    <div
                      className={`h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 border ${getRoleColor(role.name ?? '')}`}
                    >
                      <span className="text-lg font-display font-bold uppercase">
                        {role.name?.charAt(0)}
                      </span>
                    </div>
                    <div>
                      <p className="font-bold text-lg font-display capitalize flex items-center gap-2">
                        ...
                        {role.name &&
                          ['administrador', 'usuario'].includes(
                            ...
                            <span className="bg-muted px-2 py-0.5 rounded-md text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
                              Sistema
                            </span>
                          )}
                      </p>
                      <p className="text-xs font-medium text-muted-foreground mt-0.5">
                        {role.user_count ?? 0} usuarios activos
                      </p>
                    </div>
                  </div>
                  ...
                  {role.permissions?.length ? (
                    role.permissions.map((perm) => {
                      ...
                      return (
                        <span
                          key={perm}
                          className="rounded-full bg-primary/10 border border-primary/20 px-3 py-1 text-xs font-bold text-primary shadow-sm"
                        >
                          {label}
                        </span>
                      )
                    ...
                    <span className="text-xs font-bold text-muted-foreground/60 bg-muted/30 px-3 py-1 rounded-full">
