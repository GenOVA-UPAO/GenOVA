...
import { UserStatusBadge } from '@/features/admin/components/users/StatusBadge'
import { UserActionMenu } from '@/features/admin/components/users/UserActionMenu'
...
interface RoleCellProps {
  ...
  getRoleColorClasses: (roleName: string | null | undefined) => string
}
...
function formatUnivId(value: string | number | null | undefined) {
  if (!value)
    return <span className="text-muted-foreground italic text-[11px]">--</span>
  return (
    <span className="font-mono text-xs font-medium bg-muted/40 px-1.5 py-0.5 rounded border border-border/50">
...
function RoleBadge({ role }: { role: AdminUser['role'] }) {
  ...
  if (roleName === 'usuario')
    color = 'bg-accent-brand/10 text-accent-brand border-accent-brand/20'

  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider capitalize shadow-sm ${color}`}
    >
      {roleName}
    </span>
  )
        ...
        {roles.map((r) => {
          ...
          return (
            <option
              key={r.id}
              value={r.id}
              className="bg-popover text-foreground font-medium normal-case"
            >
              {r.name}
            </option>
          )
...
export function UsersTable({
  ...
  const isCurrentUserAdmin = currentUser?.role?.name === 'administrador'
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="grid grid-cols-[3rem_minmax(150px,1fr)_auto_minmax(140px,auto)_80px_100px] items-center gap-4 px-6 py-4 border-b border-border/50 bg-muted/20">
        ...
        <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground text-center hidden md:block">
          Código / Tel
        </p>
        <p className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
          Rol
        ...
        {users.length === 0 ? (
          ...
          users.map((user, i) => {
            ...
            return (
              <div
                key={user.id}
                className={`grid grid-cols-[3rem_minmax(150px,1fr)_auto_minmax(140px,auto)_80px_100px] items-center gap-4 px-6 py-4 hover:bg-accent/30 transition-colors ${i < users.length - 1 ? 'border-b border-border/50' : ''}`}
              >
                <div
                  className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-sm font-bold shadow-sm border ${avatarColor}`}
                >
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!isActive && (
                      <span className="rounded-md bg-muted px-1.5 py-0.5 text-[9px] font-bold text-muted-foreground border border-border shadow-sm uppercase tracking-widest">
                        INACTIVO
                      </span>
                    )}
                    {isMe && (
                      <span className="rounded-md bg-primary/10 text-primary px-1.5 py-0.5 text-[9px] font-bold border border-primary/20 shadow-sm uppercase tracking-widest">
                        TÚ
                      </span>
                    )}
                  </div>
                ...
                <div className="flex justify-center">
                  {isMe || isActionsDisabled ? (
                    <span className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest bg-muted/50 px-2 py-1 rounded-md border border-border/50">
                      Protegido
                    </span>
                  ) : (
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-8 shadow-sm rounded-xl border-border/60 hover:bg-accent"
                        >
                          Acción ▾
                        </Button>
                      </DropdownMenuTrigger>
